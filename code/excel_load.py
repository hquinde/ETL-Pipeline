...
class Load:
    ...
    def build_qc_records(self, qc_samples):
        ...
        for sample_id in qc_samples["Sample ID"].unique():
            ...
            for _, row in group_df.iterrows():
                group_records.append({"Sample ID": row["Sample ID"], "PPM C": row.get("PPM"), "Mean ppm C": None, "%R": None, "%RPD": None})
    ...
    def build_qcb_records(self, qcb_samples):
        ...
        for _, row in qcb_samples.iterrows():
            records.append({"Sample ID": row["Sample ID"], "PPM C": row.get("PPM"), "Mean ppm C": None, "%R": None, "%RPD": None})
        return records

    def build_qcb_average(self, qcb_samples):
        average_ppm = self.transformer.calculate_mean_ppm(qcb_samples)
        return {"Sample ID": "Average", "PPM C": average_ppm, "Mean ppm C": None, "%R": None, "%RPD": None}
    ...
    def build_sample_group_records(self, group_df):
        group_records = []
        for _, row in group_df.iterrows():
            group_records.append({"Sample ID": row["Sample ID"], "PPM C": row.get("PPM"), "Mean ppm C": None, "%RPD": None, "umol/L C": None})
    ...
    def export_all(self):
        ...
        xw.apps.active.alert("Sheets updated and formatted.", "ETL Pipeline", True)
    ...
    def format_qc_sheet(self, red_font_color):
        ...
        for row_idx in range(2, ws.api.UsedRange.Rows.Count + 1):
            sample_id_cell = ws.range((row_idx, 1))
            sample_id = sample_id_cell.value
            r_cell = ws.range((row_idx, 4))
            if r_cell.value is not None:
                check_type = 'MDL_R' if sample_id and 'MDL' in str(sample_id).upper() else 'QC_R'
